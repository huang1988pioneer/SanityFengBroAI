...
- **主題與密度**：右上角切換明場／暗場、寬距／密排；存在 `localStorage`（`fengbro.theme`、`fengbro.density`）。
  ...
  除非系統偏好是淺色。
- **值班台**：參考專案首頁是一疊黃／橘／紅底提醒卡；這版改成終端訊號列——左側緊急度色帶、等寬倒數靠右，
  三格計數同時是篩選鈕。日期一律換算成台北日曆日再相減（[`lib/attention.ts`](lib/attention.ts)）。
- **導覽**（單頁模組切換，不是一路由一模組）：
  - 桌面（≥1024px）：上方雙列——第一列鋒兄首頁／鋒兄管理／鋒兄影音／鋒兄工具／鋒兄設定（作用中是淡主色底 + 腳下發光指示條），
    第二列是該組葉片（作用中是實心青電塊）；切回某一組會還原上次葉片。
  - 平板（768–1023px）：左側圖示軌承載四組，作用中在軌內側亮一條指示線。
  - 手機（<768px）：頂部標題列 + 底部快捷（首頁／訂閱／食品／比價／更多），作用中在頂緣亮一條指示線；
    「更多」打開全部模組抽屜。
...
- 鋒兄首頁（值班台）：同時讀取訂閱、食品、試用／首購、購物清單、額度、銀行、例行，
  把落在提醒窗口內的資料排成「待處理訊號」（已過／緊急／窗口內可當篩選），點一下直接跳進該模組並帶入搜尋
- 有日期的模組在資料表最左側多一欄倒數（扣款／到期／活動／購買／額度），依最急排序；例行顯示「距上次」
- 資料表上方指標依模組而定：訂閱「7 天內扣款、每期合計（依幣別分開）」、食品「已過期、30 天內到期」、銀行「餘額合計」…
- 鋒兄訂閱
- 鋒兄試用／首購、重灌、額度、購物清單
- 鋒兄食品（商品庫存）
- 鋒兄筆記
